...
/*----------- add_new_user--------------*/
	...
	public function add_new_user($data){
		...
		$user_dep = $this->test_form_input_data($data['assign_to']);
		$user_role = 2;
		try{
			...
			if($total_email != 0 && $total_username != 0){
				...
				$add_user = $this->db->prepare("INSERT INTO tbl_admin (fullname, username, email, password, temp_password, user_role, user_dep_id) VALUES (:x, :y, :z, :a, :b, :c, :d) ");
...
/* ---------update_user_data----------*/
	...
	public function update_user_data($data, $id){
		...
		$user_dep = $this->test_form_input_data($data['assign_to']);
		try{
			$update_user = $this->db->prepare("UPDATE tbl_admin SET fullname = :x, username = :y, email = :z, user_dep_id = :a WHERE user_id = :id ");

			$update_user->bindparam(':x', $user_fullname);
			$update_user->bindparam(':y', $user_username);
			$update_user->bindparam(':z', $user_email);
			$update_user->bindparam(':a', $user_dep);
...
/* -------------admin_password_change------------*/
	...
	public function add_new_department($data){
		// data insert   
		$dep_name  = $this->test_form_input_data($data['dep_name']);
		$dep_description = $this->test_form_input_data($data['dep_description']);

		try{
			$add_department = $this->db->prepare("INSERT INTO department_info (dep_name, dep_description) VALUES (:x, :y) ");

			$add_department->bindparam(':x', $dep_name);
			$add_department->bindparam(':y', $dep_description);

			$add_department->execute();

			$_SESSION['department'] = 'department added Successfully';
	...
	public function update_department_info($data, $dep_id){
		...
		try{
			$update_department = $this->db->prepare("UPDATE department_info SET dep_name = :x, dep_description = :y WHERE dep_id = :id ");

			$update_department->bindparam(':x', $dep_name);
			$update_department->bindparam(':y', $dep_description);
			$update_department->bindparam(':id', $dep_id);

			$update_department->execute();

			$_SESSION['department'] = 'department updated Successfully';
